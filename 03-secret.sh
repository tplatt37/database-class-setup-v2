...
#
# Creates a SecretsManager Secrets for the database admin user/pass
#
...
echo "Setting up Secret to store the dbadmin password ($REGION)..."
aws cloudformation deploy --template-file core/secrets.yaml --stack-name $PREFIX-secrets --region $REGION
...
usage() {
  echo " Create a SecretsManager secrets for database admin user/pass, etc."
  echo " "
